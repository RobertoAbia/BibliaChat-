...
CREATE TABLE user_profiles (
  ...
  motive_detail TEXT,                 -- Follow-up de motive (solo analytics): family_issues, prayer_life, etc.
  consent_terms_at TIMESTAMPTZ,      -- Cuándo aceptó Términos de Servicio y Política de Privacidad
  consent_data_at TIMESTAMPTZ        -- Cuándo consintió tratamiento datos religiosos (GDPR Art. 9)
);
```
...
| `motive_detail` | TEXT | Detalle motivación (follow-up de Fe) | Key de detalle (solo analytics) | `family_issues` |
| `persistence_self_report` | BOOLEAN | Compromiso | `high` → true, `low` → false | `true` |
| `consent_terms_at` | TIMESTAMPTZ | Consentimiento (auto) | Timestamp al completar onboarding | `2026-02-17T10:30:00Z` |
| `consent_data_at` | TIMESTAMPTZ | Consentimiento (auto) | Timestamp al completar onboarding | `2026-02-17T10:30:00Z` |
...
**Valores posibles de `motive_detail`** (follow-up dinámico según `motive`, solo para Firebase Analytics):

| motive | motive_detail key | Label |
|--------|------------------|-------|
| `difficult_moment` | `family_issues` | Problemas familiares o de pareja |
| `difficult_moment` | `health_issues` | Problemas de salud |
| `difficult_moment` | `financial_issues` | Problemas económicos o laborales |
| `spiritual_growth` | `prayer_life` | Fortalecer mi vida de oración |
| `spiritual_growth` | `bible_knowledge` | Conocer mejor la Biblia |
| `spiritual_growth` | `daily_faith` | Vivir mi fe en el día a día |
| `feeling_distant` | `stopped_practicing` | Dejé de practicar mi fe |
| `feeling_distant` | `faith_doubts` | Tengo dudas sobre mi fe |
| `feeling_distant` | `painful_experience` | Una experiencia dolorosa me alejó |
| `understand_bible` | `apply_teachings` | Aplicar la Biblia a mi vida |
| `understand_bible` | `historical_context` | Entender el contexto histórico |
| `understand_bible` | `denomination_differences` | Entender diferencias entre denominaciones |
...
CREATE TABLE deleted_user_archives (
  ...
  -- Identificadores pseudonimizados (SHA256, no reversibles pero buscables)
  user_id_hash TEXT NOT NULL,      -- SHA256 del user_id (buscar por UUID)
  email_hash TEXT,                 -- SHA256 del email (buscar por email en juicios)
  ...
  -- Motivación y preferencias del onboarding
  motive TEXT,                      -- Key de motivación de fe
  motive_detail TEXT,               -- Follow-up de motivación
  features TEXT,                    -- Multi-select de apoyo (keys separadas por coma)

  -- Consentimiento GDPR (defensa legal — IMPORTANTE conservar)
  consent_terms_at TIMESTAMPTZ,    -- Cuándo aceptó Términos + Privacidad
  consent_data_at TIMESTAMPTZ,     -- Cuándo consintió datos religiosos (GDPR Art. 9)
...
CREATE INDEX idx_deleted_archives_email ON deleted_user_archives(email_hash);
...
**Campos de consentimiento (defensa legal GDPR):**

Los timestamps de consentimiento se conservan incluso después de borrar la cuenta. Si un usuario reclama que no dio consentimiento, se puede calcular su hash SHA256 y verificar cuándo aceptó:

```sql
-- Buscar por user_id (si tienes el UUID)
SELECT consent_terms_at, consent_data_at
FROM deleted_user_archives
WHERE user_id_hash = encode(digest('<user_id>', 'sha256'), 'hex');

-- Buscar por email (caso más común en juicios)
SELECT consent_terms_at, consent_data_at
FROM deleted_user_archives
...
CREATE TABLE plan_days (
  ...
  title TEXT,                          -- Título del día (ej: "Reconociendo tu situación")
...
|-------|-------------|---------|
| `title` | Título del día | "Reconociendo tu situación" |
...
**Topics de planes de motivo (4):**

| key | title | description | sort_order |
|-----|-------|-------------|------------|
| plan_momento_dificil | Plan: Momento Difícil | Plan personalizado para superar momentos difíciles | 20 |
| plan_crecimiento | Plan: Crecimiento Espiritual | Plan personalizado para crecer espiritualmente | 21 |
| plan_reconexion | Plan: Reconexión con Dios | Plan personalizado para reconectarse con Dios | 22 |
...
### Planes de Estudio (12) - Planes de Motivo del Onboarding

Planes personalizados según la motivación (`motive`) y detalle (`motive_detail`) seleccionados durante el onboarding. Cada plan tiene **7 días**. Se auto-asignan al completar el onboarding.

**UUIDs:** `b1000000-0000-0000-0000-000000000001` a `...000000000012`

| # | Motive | Detail | Nombre del Plan | Icon | Topic Key |
|---|--------|--------|----------------|------|-----------|
| 1 | difficult_moment | family_issues | Sanando relaciones familiares | 👨‍👩‍👧 | plan_momento_dificil |
| 2 | difficult_moment | health_issues | Fe en medio de la enfermedad | 🏥 | plan_momento_dificil |
| 3 | difficult_moment | financial_issues | Confianza en la provisión de Dios | 💼 | plan_momento_dificil |
| 4 | spiritual_growth | prayer_life | Fortaleciendo tu vida de oración | 🙏 | plan_crecimiento |
| 5 | spiritual_growth | bible_knowledge | Conociendo la Palabra de Dios | 📖 | plan_crecimiento |
| 6 | spiritual_growth | daily_faith | Fe en lo cotidiano | ☀️ | plan_crecimiento |
| 7 | feeling_distant | stopped_practicing | Retomando tu camino de fe | 🚶 | plan_reconexion |
| 8 | feeling_distant | doubts | Respondiendo tus dudas de fe | ❓ | plan_reconexion |
| 9 | feeling_distant | painful_experience | Sanando heridas del alma | 💔 | plan_reconexion |
| 10 | understand_bible | apply_life | La Biblia en tu vida diaria | 📋 | plan_entender_biblia |
| 11 | understand_bible | historical_context | Viajando por la historia bíblica | 🏛️ | plan_entender_biblia |
| 12 | understand_bible | denomination_differences | Entendiendo las tradiciones cristianas | ⛪ | plan_entender_biblia |

**Estructura de cada día:** Igual que los planes de pecados capitales (título, versículos, reflexión, ejercicio práctico, pregunta).
...
### Registrar token FCM (SECURITY DEFINER)

Función que bypasea RLS para manejar reasignación de tokens FCM entre usuarios (mismo dispositivo, cuenta diferente).
...
**Por qué SECURITY DEFINER:** La tabla `user_devices` tiene `UNIQUE(device_token)` y RLS con `is_owner(user_id)`. Cuando un nuevo usuario usa el mismo dispositivo, un upsert normal falla porque RLS bloquea el UPDATE de la fila que pertenece al usuario anterior. Esta función ejecuta como postgres y puede borrar/insertar sin restricciones.

**Llamada desde Flutter:** `supabase.rpc('register_device_token', params: {'p_token': token, 'p_platform': platform})`
...
| 00029_add_motive_detail_column.sql | Columna `motive_detail` en user_profiles para detalle de motivación (solo analytics) |
| 00030_add_consent_columns.sql | Columnas `consent_terms_at` y `consent_data_at` para tracking consentimiento GDPR |
| 00031_seed_motive_plans.sql | Columna `title` en plan_days + 4 topic keys de motivo + 12 planes personalizados (84 días de contenido) |
| 00032_add_archive_columns.sql | Columnas demografía + consentimiento GDPR en `deleted_user_archives` (gender, country_code, motive, motive_detail, features, consent_terms_at, consent_data_at) |
| 00033_add_email_hash_to_archives.sql | Columna `email_hash` en `deleted_user_archives` para búsqueda por email en juicios (SHA256) |
| 00034_register_device_token_function.sql | Función SQL `register_device_token()` con SECURITY DEFINER para evitar RLS violation al reusar dispositivo entre usuarios |
...
- Horario: `29 0 * * *` (00:29 AM UTC — cambiado de 06:00 a medianoche para que el evangelio esté listo desde primera hora)
...
*Topics de planes de motivo (4):*
| Key | Planes agrupados | Contexto IA |
|-----|-----------------|-------------|
| `plan_momento_dificil` | Familia, salud, finanzas | Acompañamiento en momentos difíciles |
| `plan_crecimiento` | Oración, Biblia, día a día | Crecimiento espiritual |
| `plan_reconexion` | Dejó de practicar, dudas, experiencia dolorosa | Reconexión con Dios |
| `plan_entender_biblia` | Aplicar vida, contexto histórico, denominaciones | Comprensión bíblica |
...
2. Calcular SHA256 hash del user_id y del email (pseudonimización)
3. Archivar datos en `deleted_user_archives`:
   - `user_id_hash`: SHA256 del user_id (búsqueda por UUID)
...
**Cómo buscar conversaciones de un usuario que reclama:**
```sql
-- Caso 1: El usuario da su email (lo más común en juicios)
SELECT * FROM deleted_user_archives
WHERE email_hash = encode(digest('pepe@gmail.com', 'sha256'), 'hex');

-- Caso 2: Tienes el user_id original (menos común)
SELECT * FROM deleted_user_archives
WHERE user_id_hash = encode(digest('ea677189-5c80-4117-9638-e9c7c3dbbf7c', 'sha256'), 'hex');
```
...
**Ejecución automática:**
- **GitHub Actions** cron cada hora a :29 minutos: `.github/workflows/send-notifications.yml`
- Horario cambiado de :00 a :29 para evitar ejecución temprana de GitHub Actions (GitHub puede ejecutar crons hasta 15 min tarde, no temprano)
...
**Arquitectura FCM (actualizada en build 21+):**
- **Ya NO llama a `send-notification`** como inter-function call (causaba timeouts)
- Envía FCM directamente usando FCM HTTP v1 API inline
- Incluye autenticación JWT con Service Account de Firebase
- Elimina tokens inválidos (UNREGISTERED) de la BD

**Queries corregidas (build 21+):**
- Las queries originales usaban `!inner` joins que excluían usuarios sin ciertos datos
- Cambiadas a LEFT JOINs con filtros WHERE correctos para no perder usuarios válidos

**Secrets requeridos:** `SUPABASE_SERVICE_ROLE_KEY` del entorno + `FIREBASE_SERVICE_ACCOUNT`
