...
### daily_verse_texts
Texto del versículo por versión de Biblia, con resumen coloquial generado por IA.
...
CREATE TABLE daily_verse_texts (
  ...
  verse_summary TEXT,                 -- Resumen coloquial generado por OpenAI
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
**Campo `verse_summary`:**
- Generado automáticamente por la Edge Function `fetch-daily-gospel`
- Usa OpenAI (GPT-3.5-turbo) para crear un resumen en lenguaje coloquial
- Máximo ~280 caracteres (estilo tweet)
- Ejemplo: "Isabel tuvo a su bebé Juan y todos fliparon cuando Zacarías recuperó la voz después de meses sin poder hablar."
...
Edge Function que se ejecuta diariamente para obtener el Evangelio del día.

**Flujo:**
1. Consulta **Catholic Readings API** → referencia del día (ej: "Luke 1:57-66")
2. Consulta **API.Bible** → texto en español (Reina Valera 1909)
3. Genera resumen con **OpenAI** (GPT-3.5-turbo) → lenguaje coloquial
...
| OpenAI | Generar resumen coloquial | API Key |
...
**Ejecución:**
- Manual: desde Dashboard → Edge Functions → Invoke
- Automática: configurar pg_cron para ejecutar diariamente a las 00:01 UTC
